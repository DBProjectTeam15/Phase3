...
public class CommentRepository {
    ...
    private final RowMapper<CommentDto> commentMapper = (rs, rowNum) ->
            CommentDto.builder()
                    .commentedAt(rs.getTimestamp("COMMENTED_AT") != null ?
                            rs.getTimestamp("COMMENTED_AT").toLocalDateTime() : null)
    ...
    public List<CommentDto> getMyComments(HttpSession session) {
        ...
        String sql = "SELECT * FROM COMMENTS WHERE USER_ID = ? ORDER BY COMMENTED_AT DESC";
    ...
    public List<CommentDto> getPlaylistComments(long playlistId) {
        String sql = "SELECT * FROM COMMENTS WHERE PLAYLIST_ID = ? ORDER BY COMMENTED_AT ASC";
